-- Assignment attachment must preserve the prescribed training domain.
-- A Running assignment cannot be satisfied by a Lift/Jumps/Throws workout, etc.
...
declare
  ...
  v_assignment_workout_type text;
...
begin
  ...
  select w.user_id, w.team_id, w.workout_type, w.workout_date
  into v_workout_user_id, v_workout_team_id, v_workout_type, v_workout_date
  ...
  if v_workout_type <> v_assignment_workout_type then
    raise exception 'workout training domain must match the assignment training domain'
      using errcode = '23514';
  end if;
